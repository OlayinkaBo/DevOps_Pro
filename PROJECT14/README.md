...
The objective of this course is to master the art of AWS networking implementation, including VPC, subnets, internet gateways, NAT, and routing, enabling you to design and deploy scalable and secure cloud architectures.
...
A default VPC, which Amazon provides for you in each region (think of a refion as a separate city), is like a pre-built house in that city. This house comes with some default settings to help you move in and out and start living (or start deploying your applications) immediately. But like a real house, you can change these settings according to your needs. 

Below is the step by step guide to create a new VPC:

- Choose the "VPC only" option, we'll use the "VPC and more" option later.
- Enter "first-vpc" as the name tag and "10.0.0.0/16" as the IPv4 CIDR.

The "10.0.0.0/16" will be the primary IPv4 block and you can add a secondary IPv4 block e.g., "100.64.0.0/16". The use case of secondary CIDR block could be because you're running out of IPs and need to add additional block, or there's a VPC with overlapping CIDR which you need to peer or connect. See this blog post on how a secondary CIDR block is being used in an overlapping IP scenario: [How to solve Private IP exhaustion with Private NAT Solution](https://aws.amazon.com/blogs/networking-and-content-delivery/how-to-solve-private-ip-exhaustion-with-private-nat-solution/)

![alt text](<Images/Create a new VPC.PNG>)
Leave the tags as default, you can add more tags if you want and click CREATE VPC.

As soon as the VPC is created, it's assigned with a vpc-id and there's a route table created that serves as the main route table - rtb-022b9ee623698c08e in below example
...
Go to VPC > Subnets > Create subnets and select the subnet that you have created previously - first_vpc in our example.

![alt text](Images/subnet2.PNG)

![alt text](Images/subnet3.PNG)
...
To give your public subnet access to the main road (internet), you need an Internet Gateway.
...
![alt text](<Images/IGW create steps.png>)

![alt text](<Images/IGW create steps2.PNG>)

Attach the IGW to the test-vpc

![alt text](<Images/IGW create steps3.png>)

![alt text](<Images/IGW create steps4.png>)
...
![alt text](<Images/RT Create.png>)

Put name for the route table e.g. test-vpc-public-rtb and select the desired cpc- "test-vpc

![alt text](<Images/RT Create2.png>)

Once created, edit the route table, add the default route to the Internet Gateway(IGW)

![alt text](<Images/edit routes.png>)

Next, select the internet Gateway

![alt text](<Images/edit routes2.png>)

Next, go to the **"subnet associations"** and click **"Edit subnet associations"**

![alt text](<Images/edit subnet ass.png>)

Select the public subnet and clik Save Associations

![alt text](<Images/edit subnet ass2.png>)
...
Create a NAT Gateway named "test-nat" in a private subnet. You need to allocate elastic ip because is requred for the creation of NAT gateway.

![alt text](<Images/NAT gateway2.png>)

Let's go to the route table menu and create a route table for the private subnet. Then, let's edit the route table, add a default route to the Network Address Translation (NAT) Gateway.

Choose route table Rrtb-Private, select Routes tab, and select Add Route. Under the Target, select the NAT gateway named test-nat

![alt text](<Images/add routes.png>)

![alt text](<Images/add routes2.png>)

Next, go to the "Subnet associations" tab and click "Edit subnet associations"

![alt text](<Images/edit subnet ass priv.png>)

![alt text](<Images/edit subnet ass priv2.png>)
...
By the end of this project, you'll have a clear understanding of how private subnets and NAT Gateways work in AWS and how you can use them to maintain security while allowing necessary internet access for your resources.
...
![alt text](<Images/NACL and SG.PNG>)
